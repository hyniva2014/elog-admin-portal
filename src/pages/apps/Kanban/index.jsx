...
const TaskDescription = ({
  ...
  return <Dialog open={descriptionModal} onClose={toggleDescriptionModal} maxWidth={"lg"} fullWidth>
      <DialogTitle sx={{
      display: "flex",
        ...
        my: "28px"
          ...
          <Box>
            ...
            display: "flex"
              ...
              {assignees.slice(0, 3).map((assignee, idx) => <Tooltip title={assignee.title} key={idx}>
                  <Avatar src={assignee.image} alt={assignee.title} sx={{
                height: "32px",
                width: "32px",
        ...
        <Box>
          ...
          <CustomTabPanel value={value} index={1}>
            ...
            mt: 1
              ...
              <Button size="small" variant="text" color="secondary" startIcon={<LuReply size={16} />} sx={{
              mt: "8px"
            }}>
                Reply
              </Button>
          ...
          <CustomTabPanel value={value} index={2}>
            {staticAttachments.map((file, idx) => <HorizontalFilePreview file={file} key={idx} />)}
          </CustomTabPanel>
        </Box>
      </DialogContent>
    </Dialog>;
      ...
      <Box>
        <DragDropContext onDragEnd={onDragEnd}>
          ...
          height: "calc(100vh - 165px)"
            ...
            {sections.map(section => <Droppable droppableId={section.id} key={section.id}>
                {provided => <Box ref={provided.innerRef} sx={{
              display: "flex",
              flexDirection: "column",
              pt: "16px",
              flexShrink: 0,
              ...
              borderColor: "divider"
                ...
                px: 2
                      ...
                      {selectedSection != section ? <Typography variant="h6" sx={{
                  color: "grey.800",
                  cursor: "pointer"
                }} onClick={() => setSelectedSection(section)}>
                          {section.title} ({getTasks(section.id).length})
                        </Typography> : <TextField size={"small"} value={section.title} onChange={onChangeSectionTitle} onBlur={() => setSelectedSection(null)} onKeyDownCapture={event => event.key == 'Enter' && setSelectedSection(null)} autoFocus />}
                      <IconButton onClick={() => newTask(section.id)} sx={{
                  marginRight: "8px"
                }}>
                        <LuPlus />
                      </IconButton>
                    ...
                    <SimpleBarStyled>
                      {getTasks(section.id).map((item, idx) => <Draggable draggableId={item.id + ""} index={idx} key={item.id}>
                          {provided => <Card sx={{
                    cursor: "pointer"
      ...
      <Dialog open={newTaskModal} onClose={toggleNewTaskModal} maxWidth={"md"} fullWidth>
        ...
        overflowY: "auto"
          ...
          <form onSubmit={handleNewTask}>
            <SelectInput label="Project" name="category" containerSx={{
            my: 1
          }} control={control}>
            ...
            <FormInput name="description" label="Description" containerSx={{
            my: 1,
            "& > .MuiOutlinedInput-root": {
              padding: "0px !important"
            }
          }} rows={3} control={control} multiline />
            ...
            my: 1
              ...
              <Grid item xl={6} xs={12}>
                <SelectInput name="assignTo" label="Assign To" control={control}>
                  {assignees.map((assignee, idx) => <MenuItem key={idx} value={JSON.stringify(assignee)}>
                      {assignee.title}
                    </MenuItem>)}
                </SelectInput>
              </Grid>
              ...
              <Grid item xl={6} xs={12}>
                ...
                flexDirection: "column"
                  ...
                  <FormLabel htmlFor="task-dueDate" sx={{
                  mb: 1
                }}>
                    Due Date
                  </FormLabel>

                  <CustomDatepicker hideAddon dateFormat="yyyy-MM-dd" value={newTaskDetails?.dueDate} inputClass="form-input" onChange={date => {
