...
const SidePanel = () => {
  ...
  return <>
      <Box display={"flex"} flexDirection={"column"} gap={1} id="external-events">
        ...
        {externalEvents.map((event, idx) => <Button key={idx} variant="contained" className="external-event" title={event.title} size="large" color={event.color} data-class={event.className} sx={{
        mb: "2px"
      }}>
            {event.title}
          </Button>)}
