...
import { AppBar, Box, Button, CssBaseline, Divider, Drawer, Grid, IconButton, List, ListItem, ListItemButton, ListItemIcon, ListItemText, Skeleton, SwipeableDrawer, Toolbar, Typography, styled } from "@mui/material";
import { ComponentContainerCard, PageBreadcrumb } from "@src/components";
import { toSentenceCase } from "@src/helpers";
import useDrawerToggle from "@src/hooks/useDrawerToggle";
...
const TemporaryDrawer = () => {
  ...
  return <ComponentContainerCard title="Temporary" description="Temporary navigation drawers can toggle open or closed.">
      {["left", "right", "top", "bottom"].map(anchor => <Fragment key={anchor}>
...
const SwipableDrawer = () => {
  ...
  return <ComponentContainerCard title="Swipable" description="You can make the drawer swipeable with the SwipeableDrawer component.">
      {["left", "right", "top", "bottom"].map(anchor => <Fragment key={anchor}>
          <Button onClick={toggleDrawer(anchor, true)}>{toSentenceCase(anchor)}</Button>
...
const SwipableEdgeDrawer = props => {
  const {
    window
  } = props;
  ...
  return <ComponentContainerCard title="Swipable edge" description="You can configure the SwipeableDrawer to have a visible edge when closed.">
      <Root>
        <CssBaseline />
        <Global styles={{
...
const ResponsiveDrawer = props => {
  ...
  const container = window !== undefined ? () => window().document.body : undefined;
  return <ComponentContainerCard title="Responsive" description="You can use the temporary variant to display a drawer for small screens and permanent for a drawer for wider screens.">
      <Box sx={{
      display: "flex"
    }}>
        <CssBaseline />
        ...
        ml: {
          ...
          <Toolbar>
            <IconButton color="inherit" aria-label="open drawer" edge="start" onClick={toggleOpen} sx={{
            mr: 2,
            display: {
              sm: "none"
            }
        ...
        flexShrink: {
          ...
          <Drawer container={container} variant="temporary" open={isOpen} onClose={toggleOpen} ModalProps={{
...
const PersistentDrawer = () => {
  return <ComponentContainerCard title="Persistent" description="Persistent navigation drawers can toggle open or closed.">
      <Box sx={{
      display: "flex"
    }}>
        <CssBaseline />
...
const MiniVariantDrawer = () => {
  ...
  return <ComponentContainerCard title="Mini Variant" description="In this variation, the persistent navigation drawer changes its width">
      <Box sx={{
      display: "flex"
    }}>
        <CssBaseline />
        ...
        <MuiDrawer variant="permanent" open={open}>
          ...
          <Divider />
          <List>
            {["Inbox", "Starred", "Send email", "Drafts"].map((text, index) => <ListItem key={text} disablePadding sx={{
            display: "block"
          }}>
                <ListItemButton sx={{
              minHeight: 48,
          ...
          <Divider />
          <List>
            {["All mail", "Trash", "Spam"].map((text, index) => <ListItem key={text} disablePadding sx={{
            display: "block"
          }}>
                <ListItemButton sx={{
              minHeight: 48,
...
const PermanentDrawer = () => {
  return <ComponentContainerCard title="Permanent" description="Permanent navigation drawers are always visible and pinned to the left or right edge">
      <Box sx={{
      display: "flex"
    }}>
        <CssBaseline />
...
const ClippedUnderAppbarDrawer = () => {
  return <ComponentContainerCard title="Clipped" description="Apps focused on productivity that require balance across the screen.">
      <Box sx={{
      display: "flex"
    }}>
        <CssBaseline />
