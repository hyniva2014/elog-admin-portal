...
import { Box, Checkbox, Collapse, FormControlLabel, Grid, IconButton, Paper, Switch, Table, TableBody, TableCell, TableContainer, TableFooter, TableHead, TablePagination, TableRow, TableSortLabel, Toolbar, Tooltip, Typography, alpha, styled, tableCellClasses, useTheme } from "@mui/material";
import { visuallyHidden } from "@mui/utils";
import { LuChevronDown, LuChevronFirst, LuChevronLast, LuChevronLeft, LuChevronRight, LuChevronUp, LuListFilter, LuTrash2 } from "react-icons/lu";
...
const SimpleTable = () => {
  return <ComponentContainerCard title="Simple" description="A simple example of Table with no frills.">
      <TableContainer component={Paper}>
        <Table>
          ...
          <TableBody>
            {tableRows.map(row => <TableRow key={row.name} sx={{
            "&:last-child td, &:last-child th": {
              border: 0
            }
          }}>
...
const EnhancedTable = () => {
    ...
    return <TableHead>
        <TableRow>
          <TableCell padding="checkbox">
            <Checkbox color="primary" indeterminate={numSelected > 0 && numSelected < rowCount} checked={rowCount > 0 && numSelected === rowCount} onChange={onSelectAllClick} inputProps={{
            "aria-label": "select all desserts"
          }} />
          </TableCell>
          {headCells.map(headCell => <TableCell key={headCell.id} align={headCell.numeric ? "right" : "left"} padding={headCell.disablePadding ? "none" : "normal"} sortDirection={orderBy === headCell.id ? order : false}>
  ...
  return <ComponentContainerCard title="Enhanced" description="This example demonstrates the use of Checkbox and clickable rows for selection, with a custom Toolbar">
      <Box sx={{
      width: "100%"
    }}>
        <Paper sx={{
          ...
          <TableContainer>
            ...
            minWidth: 750
              ...
              <TableBody>
                ...
                return <TableRow hover onClick={event => handleClick(event, row.name)} role="checkbox" aria-checked={isItemSelected} tabIndex={-1} key={row.name} selected={isItemSelected} sx={{
                  cursor: "pointer"
                }}>
                      <TableCell padding="checkbox">
...
const CustomizedTable = () => {
  return <ComponentContainerCard title="Customized" description="Here is an example of customizing the component.">
      <TableContainer component={Paper}>
        <Table sx={{
        minWidth: 700
          ...
          <TableBody>
            {tableRows.map(row => <StyledTableRow key={row.name}>
...
const CustomPagination = () => {
  ...
  return <ComponentContainerCard title="Custom Pagination" description="The ActionsComponent prop of the TablePagination component allows the implementation of custom actions.">
      <TableContainer component={Paper}>
        <Table sx={{
        minWidth: 500
      }} aria-label="custom pagination table">
          <TableBody>
            {(rowsPerPage > 0 ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : rows).map(row => <TableRow key={row.name}>
                <TableCell component="th" scope="row">
                  {row.name}
                </TableCell>
                <TableCell style={{
...
const StickyHeader = () => {
  ...
  return <ComponentContainerCard title="Sticky Header" description="Use stickyHeader prop to make header sticky">
      <Paper sx={{
      width: "100%",
      overflow: "hidden"
    }}>
...
const ColumnGrouping = () => {
  ...
  return <ComponentContainerCard title="Column Grouping" description="You can group column headers by rendering multiple table rows inside a table head:">
      <Paper sx={{
      width: "100%"
    }}>
        <TableContainer sx={{
...
const CollapsibleTable = () => {
  ...
  return <ComponentContainerCard title="Collapsible Rows" description="Collapsible Rows, by using Collapse Component">
      <TableContainer component={Paper}>
...
const SpanningTable = () => {
  ...
  return <ComponentContainerCard title="Spanning" description="A simple example with spanning rows & columns.">
      <TableContainer component={Paper}>
        <Table sx={{
        minWidth: 700
