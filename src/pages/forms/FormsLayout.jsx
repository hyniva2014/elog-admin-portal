...
import { CheckboxInput, ComponentContainerCard, FormInput, PageBreadcrumb, PasswordInput, SelectInput } from "@src/components";
      ...
      <ComponentContainerCard title="Basic Example">
        <Box pt={"20px"} width={"100%"}>
          <form onSubmit={handleSubmit(() => null)}>
            <FormInput name="email" label="Email Address" containerSx={{
            mb: "12px"
          }} type="email" placeholder="Enter email" control={control} />
            <PasswordInput name="password" label="Password" containerSx={{
            mb: "12px"
          }} placeholder="Enter password" control={control} />
...
const HorizontalForm = () => {
  const loginFormSchema = yup.object({
    ...
    retypepassword2: yup.string().oneOf([yup.ref("password2")], "Passwords must match").required("Enter Re Password"),
    rememberMe: yup.boolean().oneOf([true], "Checkbox must be checked").optional()
  });
  const {
    control,
...
const SizingForm = () => {
  ...
  return <Grid item xs={12}>
      <ComponentContainerCard title="Basic Example" description="As shown in the previous examples, our grid system allows you to place any number of a .grid-cols-* and .flex">
        <Box pt={"20px"} width={"100%"}>
          <form onSubmit={handleSubmit(() => null)}>
          ...
          <form onSubmit={handleSubmit(() => null)}>
            ...
            gap: "16px"
              ...
              <FormInput name="username" placeholder="Username" control={control} startAdornment={<InputAdornment position="start">
                    <LuAtSign />
                  </InputAdornment>} />
              <Box>
                <Button type="submit" variant="contained" color="primary" size="large">
...
const GridForm = () => {
  ...
  return <Grid item xs={12}>
      <ComponentContainerCard title="Grid" description="More complex layouts can also be created with the grid system.">
        <form onSubmit={handleSubmit(() => null)} style={{
        width: "100%"
      }}>
