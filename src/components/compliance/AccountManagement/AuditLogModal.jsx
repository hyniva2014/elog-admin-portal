...
const renderCreatedOnCell = (params) => <CreatedOnCell params={params} />;
...
const AuditLogModal = ({
  ...
  const rows = useMemo(
    () =>
      auditData.rows?.map((entry, index) => AuditLogRow({ entry, index })) ||
      [],
    [auditData.rows],
  );
